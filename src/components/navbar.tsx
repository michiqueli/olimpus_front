...
const NavBar: React.FC = () => {
  ...
  // Supongamos que orderDetails está en el estado del componente
  const [orderDetails, setOrderDetails] =
    useState(/* tu lógica para obtener o inicializar orderDetails */);
  ...
  const handleStartPurchase = async () => {
    try {
      // Realiza una solicitud al servidor para obtener la URL de pago
      const response = await fetch("/mercadopago/create-preference", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(orderDetails), // Puedes enviar detalles del pedido si es necesario
      });

      const data = await response.json();

      // Redirige al usuario a la URL de pago en MercadoPago
